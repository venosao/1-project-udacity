...
#                                                                             
# PROGRAMMER: Olga Venosa
# DATE CREATED:   30/05/2019                              
# REVISED DATE: 06/06/2019
...
def classify_images(images_dir, results_dic, model):
 ...
 # Process all files in the results_dic - use images_dir to give fullpath
    ...
    for key in results_dic:
       ...
       found = model_label.find(truth[0])
       ...
       if found >= 0:
           if ( (found == 0 and len(truth)==len(model_label)) or
               ...
               if key not in results_dicall:
                   results_dicall[key] = [truth, model_label, 1]
                   
           # found within a word/term not a label existing on its own 
           else:
               if key not in results_dicall:
                   results_dicall[key] = [truth, model_label, 0]
